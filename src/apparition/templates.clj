...
(defn base [template & [params]] (html5
  [:head [:title "Welcome to apparition" ]]
  [:body
  [:div.navbar.navbar-inverse.navbar-fixed-top [:div.container
    [:div.navbar-header [:a.navbar-brand {:href "/"} "apparition"]]
    [:div.navbar-collapse.collapse [:ul.nav.navbar-nav
      (for [[el text sub] [["home" "Home" ""] ["about" "About"] ["some" "Some Dev Work"]]]
        (let [sub (if (nil? sub) el sub)]
          [:li {:class (str el "-selected")} [:a {:href (str "/" sub)} text]]))]]]]
...
(defn some-page [& [params]] (base [:p [:h1 "Hello, Bro 1!!!"]] params))
